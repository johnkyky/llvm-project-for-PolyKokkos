...
/// function name.
static std::string getUniqueScopName(const Scop *S) {
...
static __isl_give isl_id_to_ast_expr *pollyBuildAstExprForStmt(
    void *StmtT, __isl_take isl_ast_build *BuildC,
...
public:
  ...
  std::string getKernelFuncName(int KernelId);
...
std::string GPUNodeBuilder::getKernelFuncName(int KernelId) {
  return "FUNC_" + S.getFunction().getName().str() + "_SCOP_" +
         std::to_string(S.getID()) + "_KERNEL_" + std::to_string(KernelId);
}
...
void GPUNodeBuilder::prepareManagedDeviceArrays() {
  ...
  for (int I = 0; I < Prog->n_array; ++I) {
    ...
    Value *Offset = getArrayOffset(Array);
    if (Offset) {
      HostPtr = Builder.CreatePointerCast(HostPtr, Builder.getPtrTy());
      HostPtr = Builder.CreateGEP(ScopArray->getElementType(), HostPtr, Offset);
    }

    // TODO: Ugo to check Builder.getInt8PtrTy() -> Builder.getPtrTY()
    HostPtr = Builder.CreatePointerCast(HostPtr, Builder.getPtrTy());
...
Value *GPUNodeBuilder::createCallGetKernel(Value *Buffer, Value *Entry) {
  ...
  if (!F) {
    ...
    Args.push_back(Builder.getPtrTy());
    // TODO: Ugo to check Builder.getInt8PtrTy() -> Builder.getPtrTY()
    FunctionType *Ty = FunctionType::get(Builder.getPtrTy(), Args, false);
...
Value *GPUNodeBuilder::getArraySize(gpu_array_info *Array) {
  ...
  if (!gpu_array_is_scalar(Array)) {
    ...
    for (unsigned int I = 1; I < Array->n_index; I++) {
      isl::pw_aff BoundI = ArrayBound.at(I);
      isl::ast_expr Expr = Build.expr_from(BoundI);
      Res = Res.mul(Expr);
    }
...
Value *GPUNodeBuilder::getArrayOffset(gpu_array_info *Array) {
  ...
  for (unsigned I : rangeIslSize(0, Min.tuple_dim())) {
    if (I > 0) {
      ...
      isl::ast_expr BExpr = Build.expr_from(BoundI);
...
// TODO Ugo check if the function is correct
void GPUNodeBuilder::createKernelSync() {
  ...
  case GPUArch::SPIR32: {
    ...
    Function *Sync = M->getFunction(SpirName);
...
static isl_bool collectReferencesInGPUStmt(__isl_keep isl_ast_node *Node,
                                           void *User) {
...
static std::string getCUDALibDeviceFuntion(StringRef NameRef) {
...
void GPUNodeBuilder::insertStoreParameter(Type *ArrayTy,
  ...
  // TODO: Ugo to check Builder.getInt8PtrTy() -> Builder.getPtrTY()
  Value *ParamTyped = Builder.CreatePointerCast(Param, Builder.getPtrTy());
...
GPUNodeBuilder::createLaunchParameters(ppcg_kernel *Kernel, Function *F,
  ...
  if (Runtime == GPURuntime::OpenCL)
    // TODO: Ugo to check Builder.getInt8PtrTy() -> Builder.getPtrTY()
    ArrayTy = ArrayType::get(Builder.getPtrTy(), 2 * NumArgs);
  else
    // TODO: Ugo to check Builder.getInt8PtrTy() -> Builder.getPtrTY()
    ArrayTy = ArrayType::get(Builder.getPtrTy(), NumArgs);
  ...
  Instruction *Parameters =
      new AllocaInst(ArrayTy, AddressSpace, Launch + "_params",
                     EntryBlock->getTerminator()->getIterator());
  ...
  for (long I = 0; I < Prog->n_array; I++) {
    ...
    if (Offset) {
      DevArray = Builder.CreatePointerCast(DevArray, Builder.getPtrTy());
    ...
    if (gpu_array_is_read_only_scalar(&Prog->array[I])) {
      ...
      // TODO: Ugo to check Builder.getInt8PtrTy() -> Builder.getPtrTY()
      Value *ValPtrCast = Builder.CreatePointerCast(ValPtr, Builder.getPtrTy());
      Builder.CreateStore(ValPtrCast, Slot);
    } else {
      // TODO: Ugo to check Builder.getInt8PtrTy() -> Builder.getPtrTY()
      Instruction *Param =
          new AllocaInst(Builder.getPtrTy(), AddressSpace,
  ...
  auto Location = EntryBlock->getTerminator()->getIterator();
  // TODO: Ugo to check Builder.getInt8PtrTy() -> Builder.getPtrTY()
  return new BitCastInst(Parameters, Builder.getPtrTy(),
...
void GPUNodeBuilder::createKernel(__isl_take isl_ast_node *KernelStmt) {
  ...
  Value *KernelString = Builder.CreateGlobalString(ASMString, Name);
  Value *NameString = Builder.CreateGlobalString(Name, Name + "_name");
...
GPUNodeBuilder::createKernelFunctionDecl(ppcg_kernel *Kernel,
  ...
  for (long I = 0; I < Prog->n_array; I++) {
    ...
    if (gpu_array_is_read_only_scalar(&Prog->array[I])) {
      ...
      // TODO: Ugo to check Builder.getInt8PtrTy() -> Builder.getPtrTY()
      Args.push_back(Builder.getPtrTy(UseGlobalMemory));
      MemoryType.push_back(
...
void GPUNodeBuilder::insertKernelIntrinsics(ppcg_kernel *Kernel) {
  ...
  auto AddId = [this](__isl_take isl_id *Id, Intrinsic::ID Intr) mutable {
    ...
    Module *M = Builder.GetInsertBlock()->getParent()->getParent();
    Function *IntrinsicFn = Intrinsic::getOrInsertDeclaration(M, Intr);
...
void GPUNodeBuilder::createKernelFunction(
  ...
  case GPUArch::NVPTX64:
    if (Runtime == GPURuntime::CUDA)
      GPUModule->setTargetTriple(Triple("nvptx64-nvidia-cuda"));
    else if (Runtime == GPURuntime::OpenCL)
      GPUModule->setTargetTriple(Triple("nvptx64-nvidia-nvcl"));
    GPUModule->setDataLayout(computeNVPTXDataLayout(true /* is64Bit */));
    break;
  case GPUArch::SPIR32:
    GPUModule->setTargetTriple(Triple("spir-unknown-unknown"));
    GPUModule->setDataLayout(computeSPIRDataLayout(false /* is64Bit */));
    break;
  case GPUArch::SPIR64:
    GPUModule->setTargetTriple(Triple("spir64-unknown-unknown"));
...
void GPUNodeBuilder::addCUDALibDevice() {
  ...
  if (requiresCUDALibDevice()) {
    ...
    LibDeviceModule->setTargetTriple(Triple("nvptx64-nvidia-cuda"));
...
namespace {
class PPCGCodeGeneration final {
public:
  ...
  isl_union_map *
  getTaggedAccesses(enum polly::MemoryAccess::AccessType AccessTy) {
  ...
  isl_union_map *getTaggedReads() {
    return getTaggedAccesses(polly::MemoryAccess::READ);
  }
  ...
  isl_union_map *getTaggedMayWrites() {
    return isl_union_map_union(
        getTaggedAccesses(polly::MemoryAccess::MAY_WRITE),
        getTaggedAccesses(polly::MemoryAccess::MUST_WRITE));
  }
  ...
  isl_union_map *getTaggedMustWrites() {
    return getTaggedAccesses(polly::MemoryAccess::MUST_WRITE);
  }
  ...
  gpu_gen *generateGPU(ppcg_scop *PPCGScop, gpu_prog *PPCGProg) {
    ...
    if (!HasPermutable || HasPermutable < 0) {
  ...
  bool run(Scop &Scop, LoopInfo &LInfo, DominatorTree &DTree,
    ...
    if (containsInvalidKernelFunction(*S, Architecture == GPUArch::NVPTX64)) {
      LLVM_DEBUG(
          dbgs() << getUniqueScopName(S)
