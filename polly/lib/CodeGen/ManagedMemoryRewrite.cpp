...
#include "polly/CodeGen/ManagedMemoryRewrite.h"
...
static void expandConstantExpr(ConstantExpr *Cur, PollyIRBuilder &Builder,
  ...
  Instruction *Inst = Cur->getAsInstruction();
  assert(Inst && "unable to convert ConstantExpr to Instruction");
  ...
  for (unsigned I = 0; I < Inst->getNumOperands(); I++) {
    ...
    if (ConstantExpr *CExprOp = dyn_cast<ConstantExpr>(Op))
      expandConstantExpr(CExprOp, Builder, Inst, I, Expands);
  }
}
...
replaceGlobalArray(Module &M, const DataLayout &DL, GlobalVariable &Array,
  ...
  GlobalVariable *ReplacementToArr =
      cast<GlobalVariable>(M.getOrInsertGlobal(NewName, Builder.getPtrTy()));
  ReplacementToArr->setInitializer(
  ...
  Value *AllocatedMemTyped = Builder.CreatePointerCast(
      AllocatedMemRaw, Builder.getPtrTy(), "mem.typed");
  ...
  for (Instruction *UserOfArrayInst : ArrayUserInstructions) {
    ...
    // <ty>** -> <ty>*
    Value *ArrPtrLoaded =
        Builder.CreateLoad(Builder.getPtrTy(), ReplacementToArr, "arrptr.load");
    // <ty>* -> [ty]*
    Value *ArrPtrLoadedBitcasted = Builder.CreateBitCast(
        ArrPtrLoaded, Builder.getPtrTy(), "arrptr.bitcast");
...
class ManagedMemoryRewritePass final {
public:
  GPUArch Architecture;
  GPURuntime Runtime;

  ManagedMemoryRewritePass() {}
  bool run(Module &M) {
...
bool runManagedMemoryRewrite(Function &F, GPUArch Arch, GPURuntime Runtime) {
  ...
  ManagedMemoryRewritePass *MMR = new ManagedMemoryRewritePass();
  MMR->Runtime = Runtime;
  MMR->Architecture = Arch;

  return MMR->run(*F.getParent());
}
