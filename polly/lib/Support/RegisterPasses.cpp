...
static void buildEarlyPollyPipeline(llvm::ModulePassManager &MPM,
  ...
  MPM.addPass(MarkFunctionToFindScop());
...
static void buildPrepareForLatePollyPipeline(llvm::ModulePassManager &MPM,
  ...
  MPM.addPass(MarkFunctionToFindScop());
  // MPM.addPass(ModulePassTest());
  MPM.addPass(FunctionMarkedInliner());
...
///
/// The default is currently a), to register Polly such that it runs as early
/// as possible. This has several implications:
///
///   1) We need to schedule more canonicalization passes
///
///   As nothing is run before Polly, it is necessary to run a set of
///   preparing transformations before Polly to canonicalize the LLVM-IR and
///   to allow Polly to detect and understand the code.
///
///   2) We get the full -O3 optimization sequence after Polly
///
///   The LLVM-IR that is generated by Polly has been optimized on a high
///   level, but it may be rather inefficient on the lower/scalar level. By
///   scheduling Polly before all other passes, we have the full sequence of
///   -O3 optimizations behind us, such that inefficiencies on the low level
